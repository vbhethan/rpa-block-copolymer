...
def _():
    ...
    from rpa.optimizers import optimize_box_only, optimize_phi_only, optimize_joint
    from rpa.optimizers import scipy_optimize_phi_only, scipy_optimize_joint, scipy_optimize_box_only
...
def _(
    ...
    return initial_data, phi_A_raw, phi_B_raw, samples


@app.cell
def _(initial_data, plot_diblock_isosurface):
    fig_initial = plot_diblock_isosurface(
        initial_data, frame=0, tile=1, periodic_pad=1, title="Loaded density"
    )
    fig_initial
    return
...
def _(plot_diblock_isosurface, result_box):
    fig_box = plot_diblock_isosurface(
        result_box, frame=-1, tile=1, periodic_pad=1, title="After box optimization"
    )
    fig_box
    return
...
def _(plot_diblock_isosurface, result_phi):
    fig_phi = plot_diblock_isosurface(
        result_phi, frame=-1, tile=1, periodic_pad=1, title="After phi optimization"
    )
    fig_phi
    return
...
def _(
    ...
    # samples are in [-1, 1]; map to physical density in [0, 1]
    _phi_A_raw = (samples[1, 0] + 1.0) / 2.0   # (32, 32, 32)
    _phi_B_raw = 1.0 - phi_A_raw
    _phi = torch.stack([phi_A_raw, phi_B_raw], dim=0)  # (2, 32, 32, 32)
    _box_lengths = [20,20,20]

    _model = BlockCopolymerFreeEnergy(
        N=N,
        block_fractions=block_fractions,
        chi_matrix=chi_matrix,
        l_ij_matrix=l_ij_matrix,
    ...
    _model.set_density(_phi*0.1)

    # Optimize box size
    sample_optim = optimize_joint(_model)
    return (sample_optim,)


@app.cell
def _(plot_diblock_isosurface, sample_optim):
    plot_diblock_isosurface(sample_optim, tile=1)
    return
...
def _(
    ...
    # samples are in [-1, 1]; map to physical density in [0, 1]
    _phi_A_raw = (samples[1, 0] + 1.0) / 2.0   # (32, 32, 32)
    _phi_B_raw = 1.0 - _phi_A_raw
    _phi = torch.stack([_phi_A_raw, _phi_B_raw], dim=0)  # (2, 32, 32, 32)
    _box_lengths = [10,10,10]
    ...
    _model.set_density(_phi)

    _result = scipy_optimize_phi_only(_model, method="BFGS")
    plot_diblock_isosurface(_result, tile=1)
    return
